...
def get_detection_data_vehicle (dataset_id, data_dict, annotation_dict):
    ...
    for idx_dataset, dataset in enumerate(dataset_id):
        vehicle_dataset = CocoCarDataset(annotation_dict[dataset], train_pipeline)
        data_infos = vehicle_dataset.data_infos
        anno_infos = vehicle_dataset.anno_infos

        dataset_id_list = [idx_dataset + 1 for n in range(len(data_infos))]
        dataset_ids.extend (dataset_id_list)

        # detection_results = []
        # print (len (data_infos), len (anno_infos))
        for idx_img, img_info in enumerate(data_infos):
            fname = data_dict[dataset] + img_info['file_name']
            img_paths.append (fname)
            annotations.append (anno_infos[idx_img])
            ret.append((fname, anno_infos[idx_img], idx_dataset + 1))
    
        print("ID", idx_dataset + 1, "dataset", dataset, "loaded")
        print("  subset   | # images")
        print("  -----------------------")
        print("  train    | {:8d}"
                .format(len (data_infos)))

    print ("whole dataset contains", total_ids, "ids", len (img_paths), "images")
    return img_paths, annotations, np.array(dataset_ids), ret
